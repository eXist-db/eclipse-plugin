 ...
 * <p>
 * Used for additional space after comma in parameter list, parameter type
 * support and custom {@link XQueryProposalContextInformation}.
 * 
...
public class XQueryScriptMethodCompletionProposal extends
	...
	private static final int XQUERY_SPECIFIC_2 = 2;
	...
	public XQueryScriptMethodCompletionProposal(CompletionProposal proposal,
	...
	public void apply(IDocument document, char trigger, int offset) {
		...
		if (fArgumentOffsets != null && getTextViewer() != null) {
			try {
				...
				LinkedModeUI ui = new LinkedModeUI(model, getTextViewer());
		...
		if (hasParameters()) {
			...
			for (int i = 0; i < findParameterNames.length;) {
				...
				if (i < findParameterNames.length) {
					buffer.append(COMMA).append(XQUERY_SPECIFIC);
				}
			}
